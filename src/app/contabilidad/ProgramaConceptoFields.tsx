...
type Concepto = ConceptoConfigurable & {
  id_concepto: number;
...
export function ProgramaConceptoFields({ programas, conceptos, programaInicial, conceptoInicial }: {
  ...
  const opciones = useMemo(
    () => programa ? conceptos.filter((c) => conceptoPermitido(programa.tipo_programa, c)) : conceptos,
    [conceptos, programa]
  );
  const aviso = programa && conceptoActual ? avisoConcepto(programa.tipo_programa, conceptoActual) : null;
  ...
  return <>
    <label style={labelStyle}>
      ...
      {conceptoActual?.requisito_entidad_origen === "obligatoria" ? <span style={{ color: "#92580a" }}>Requiere entidad de origen.</span> : null}
      {conceptoActual?.requisito_descripcion === "obligatoria" ? <span style={{ color: "#92580a" }}>Requiere descripción en observaciones.</span> : null}
    </label>
    <label style={labelStyle}>
      Programa
      <select name="programa_id" value={programaId} style={fieldStyle} onChange={(e) => {
        ...
        if (nextPrograma && conceptoActual && !conceptoPermitido(nextPrograma.tipo_programa, conceptoActual)) {
